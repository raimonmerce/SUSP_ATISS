...
def render_to_folder(
    ...
    bbox_params_t = torch.cat(
        [
            boxes["class_labels"].double(),
            boxes["translations"].double(),
            boxes["sizes"].double(),
            boxes["angles"].double()
        ],
        dim=-1
    ...
    if add_start_end:
        bbox_params_t = torch.cat([
            torch.zeros(1, 1, bbox_params_t.shape[2]).double(),
            bbox_params_t.double(),
            torch.zeros(1, 1, bbox_params_t.shape[2]).double(),
        ], dim=1)
...
def sample_in_bbox(class_probs, translation_probs, bbox, trials=1000):
    ...
    class_probs = class_probs.cpu().numpy().ravel()
    translation_probs = [
        [
